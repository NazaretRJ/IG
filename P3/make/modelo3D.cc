...
void modelo3D::reservar(int num_vertices, int num_cara){
  ...
  //inicializamos los colores
	Colores(false);//inicializamos colores normales
  Colores(true);//inicializamos color chess
}

void modelo3D::Colores(bool chess){

	/*	cout<<"size v"<<vertices.size()<<endl;
			cout<<" colores"<<endl;
	cout<<"size"<<color_vert.size()<<endl;*/
	color_vert.resize(vertices.size());
  color_chess_izq.resize(vertices.size());
  color_chess_der.resize(vertices.size());
//porqué en el chess se vuelven negros en objeto revolución?
	if(!chess){
	...
	/*cout<<"size v"<<vertices.size()<<endl;
cout<<"size despues"<<color_vert.size()<<endl;
	for(int i = 0 ; i<color_vert.size();i++){
  cout<<color_vert[i].r<<" "<<color_vert[i].g<<" "<<color_vert[i].b<<" "<<endl;
}*/
...
void modelo3D::draw(GLenum mode){
	glPointSize(4);
...
void modelo3D::Escalar(float n){
  if(n!=0){
    for(int i = 0;i<vertices.size();i++){
      vertices[i].x=vertices[i].x*n;
      vertices[i].y=n*vertices[i].y;
      vertices[i].z=n*vertices[i].z;
    }
  }
