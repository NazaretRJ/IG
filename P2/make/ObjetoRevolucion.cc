...
Puntos ObjetoRevolucion::RotarX(Puntos &v,float ang){
  Puntos aux;
  ang=(ang*PI)/180;
  aux.x=v.x;
  aux.y=v.y*cos(ang) - v.z*sin(ang);
  aux.z= v.y* sin(ang) + v.z*cos(ang);
  return aux;

}

Puntos ObjetoRevolucion::RotarZ(Puntos &v,float ang){
  Puntos aux;
  ang=(ang*PI)/180;
  aux.x=v.x*cos(ang) - v.y*sin (ang);
  aux.y=v.x*sin(ang) + v.y*cos(ang);
  aux.z=v.z;
  return aux;
}
...
if(v.back().x==0 && v.back().z==0){//esta sobre eje y
  ...
  for(int k=1;k<n_rotaciones;k++){
    if(Y==true){
      ...
      if(Z==true)
          vertices.push_back(RotarZ(v[0],ang_actual));
      else
          vertices.push_back(RotarX(v[0],ang_actual));
    ...
    for(int j=1;j<v.size();j++){
      ...
      if(Y==true){
        ...
        if(Z==true)
            vertices.push_back(RotarZ(v[j],ang_actual));
        else
            vertices.push_back(RotarX(v[j],ang_actual));
